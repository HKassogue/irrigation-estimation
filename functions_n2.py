...
def make_plots(h_true, z_true, z_est, h_est, t, save=False, outdir = "figs"):
...
def q2_sensibility(A, G2, C, z0, h1, h2, T=1, N=200, q1=1, q2_list=[0.25, 0.5, 1], save=False, outdir = "figs"):
    ...
    def make_plot_over_t_per_q2(values_true, values_est, label, ylabel, title=None, save=False, filename=None):
        ...
        if save:
            plt.savefig(os.path.join(outdir, filename), dpi=200); plt.close()
        else:
            plt.show()

    # --- Utilitary function to plot/save the metrics over q2 and anotate it ---
    def make_plot_over_q2_and_anotate(x, y, xlabel, ylabel, title=None, save=False, filename=None):
        plt.figure(figsize=(8, 5))
    ...
    make_plot_over_q2_and_anotate('misfit', 'energy', 
                                    ...
                                    save=save, filename="fig_n2_misfit_energy_q2_sensibility.png")
...
def q1_sensibility(A, G2, C, z0, h1, h2, T=1, N=200, q1_list=[0.25, 0.5, 1], q2=1, save=False, outdir = "figs"):
    ...
    def make_plot_over_t_per_q1(values_true, values_est, label, ylabel, title=None, save=False, filename=None):
        plt.figure(figsize=(8, 5))
        plt.plot(t, values_true, "k-", label=label)
    ...
    def make_plot_over_q1_and_anotate(x, y, xlabel, ylabel, title=None, save=False, filename=None):
        plt.figure(figsize=(8, 5))
        plt.plot(metrics[x], metrics[y], marker="o")
        for i, row in metrics.iterrows():
    ...
    make_plot_over_t_per_q1(h_true[:, 0], curves_h1, 
                            ...
                            save=save, filename="fig_n2_h1_q1_sensibility.png")
    make_plot_over_t_per_q1(h_true[:, 1], curves_h2, 
                            label=f"h2 true", 
                            ylabel=r"$h_2(t)$", 
                            title="Sensibility in q1 of h2", 
                            save=save, filename="fig_n2_h2_q1_sensibility.png")
    make_plot_over_t_per_q1(z_true[:, 0], curves_z1, 
                            label=f"z1 true", 
                            ylabel=r"$z_1(t)$", 
                            title="Sensibility in q1 of z1", 
                            save=save, filename="fig_n2_z1_q1_sensibility.png")
    make_plot_over_t_per_q1(z_true[:, 1], curves_z2, 
                            label=f"z2 true", 
                            ylabel=r"$z_2(t)$", 
                            title="Sensibility in q1 of z2", 
                            save=save, filename="fig_n2_z2_q1_sensibility.png")
    make_plot_over_q1_and_anotate('q1', 'z0_error', 
                                    xlabel='q1', 
                                    ylabel=r"$z_{0Error}=|z_0 - z_0^\star|$", 
                                    title=r"Error of $z_0$", 
                                    save=save, filename="fig_n2_z0_error_q1_sensibility.png")
    make_plot_over_q1_and_anotate('q1', 'z0_energy', 
                                    xlabel='q1', 
                                    ylabel=r"$z_{0Energy}=|z_0^\star|$", 
                                    title=r"Energy of $z_0$", 
                                    save=save, filename="fig_n2_z0_energy_q1_sensibility.png")
    make_plot_over_q1_and_anotate('q1', 'z_rmse', 
                                    xlabel='q1', 
                                    ylabel=r"$RMSE_z$", 
                                    title=r"$RMSE_z$", 
                                    save=save, filename="fig_n2_z_rmse_q1_sensibility.png")
    make_plot_over_q1_and_anotate('q1', 'h_rmse', 
                                    xlabel='q1', 
                                    ylabel=r"$RMSE_h$", 
                                    title=r"$RMSE_h$", 
                                    save=save, filename="fig_n2_h_rmse_q1_sensibility.png")
    make_plot_over_q1_and_anotate('z_rmse', 'h_rmse', 
                                    xlabel=r"$RMSE_z$", 
                                    ylabel=r"$RMSE_h$", 
                                    title=r"Compromis $RMSE_z$/$RMSE_h$", 
                                    save=save, filename="fig_n2_z_h_rmse_q1_sensibility.png")
    make_plot_over_q1_and_anotate('q1', 'misfit', 
                                    xlabel='q1', 
                                    ylabel=r"$\mathrm{YMisfit}=\int_0^T\|Cz(t)-y^m(t)\|^2 dt$", 
                                    title="Y Misfit as function of q1", 
                                    save=save, filename="fig_n2_misfit_q1_sensibility.png")
    make_plot_over_q1_and_anotate('q1', 'energy', 
                                    xlabel='q1', 
                                    ylabel=r"$\mathrm{HEnergy}=\int_0^T \|h(t)\|^2 dt$", 
                                    title="H energy as function of q1", 
                                    save=save, filename="fig_n2_energy_q1_sensibility.png")
    make_plot_over_q1_and_anotate('misfit', 'energy', 
                                    xlabel=r"$\mathrm{YMisfit}=\int_0^T\|Cz(t)-y^m(t)\|^2 dt$", 
                                    ylabel=r"$\mathrm{HEnergy}=\int_0^T \|h(t)\|^2 dt$", 
                                    title="Compromis YMisfit/HEnergy", 
                                    save=save, filename="fig_n2_misfit_energy_q1_sensibility.png")
...
def q12_sensibility_with_optimum(A, G2, C, z0, h1, h2, T=1, N=200, q1_list=(0.25, 0.5, 1), q2_list=(0.25, 0.5, 1), save=False, outdir="figs"):
    """
    ...
    for i, q1 in enumerate(q1_list):
        for j, q2 in enumerate(q2_list):
            ...
            metrics_list.append({
                ...
                "misfit": misfit,
                "energy": energy,
                "objective": misfit + q1 * z0_energy + q2 * energy
            })

    metrics = pd.DataFrame(metrics_list).sort_values(["q1", "q2"]).reset_index(drop=True)

    # --- Compromise criterion J(q1, q2) = yMisfit + q1 * z0Energy + q2 * hEnergy ---
    # metrics["objective"] = metrics["misfit"] + q1 * metrics["z0_energy"] + q2 * metrics["energy"]

    # --- Optimal point : (q1*, q2*) minimising J ---
    ...
    plot_surface_metric("energy",    r"$\mathrm{HEnergy}=\int_0^T \|h(t)\|^2 dt$",  "surf_n2_energy_q1_q2_sensibility.png")
    ...
    plot_contour_metric("energy",    r"$\mathrm{HEnergy}=\int_0^T \|h(t)\|^2 dt$",  "contour_n2_energy_q1_q2_sensibility.png")
    # Contours 2D du critère J
    plot_contour_metric("objective", r"$J(z_0,h)$ as function of $(q_1, q_2)$",     "contour_n2_objective_q1_q2.png")
